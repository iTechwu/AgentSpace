...
审查基线：`dev` 分支 `1199e166`，2026-08-17。此清单区分“已有 Prisma 写适配器”和“仍由同步 PostgreSQL/SQLite 路径承担”的事实，不把 Prisma model 存在误判为写迁移完成。
...
| --- | --- | --- | --- | --- |
| P1 | task enqueue：`workflows/dispatcher.ts` → `enqueueNativeTaskSync` | 事务内先 claim node，再创建 `agent_task_queue`；失败时回滚 node 状态 | idempotency key、claim/queue 原子性、队列不可用重试、affected rows、重复 dispatch | 先建 Prisma transaction wrapper，再做 shadow write |
...
| P1 | workflow outbox：`workflows/outbox.ts`、`workflows/definitions.ts` | `INSERT INTO workflow_outbox` 与业务变更需同事务 | outbox idempotency、delivery lease、attempt/backoff、事件 payload digest | 单独迁移，先比对未投递数量和重复消费 |
| P1 | workflow lease/recovery：`workflows/leases.ts`、reconcile worker | lease 更新和 worker reclaim 依赖时间条件与 affected rows | owner token、expiresAt、compare-and-swap、抢占失败分类 | 先建立并发锁压测，再开写 flag |
...
本 inventory 已完成“发现和契约定义”，并不表示待迁移路径已经实施。下一批建议只选择 task enqueue + workflow outbox，先完成事务/幂等/事件顺序对照，再扩展 lease 和权限域；在这些证据完成前，不删除同步 legacy 写路径。
