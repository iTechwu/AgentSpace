...
本 inventory 已完成“发现、契约定义、dispatcher/run fan-out/materialization 接线、coordinator ready/outbox 原语和真实 deadlock 注入”。它仍不表示生产写切流已经实施：coordinator 仍处于外层同步 task-commit 事务内，recovery lease 和 30 天 shadow 对照仍缺失；在这些证据完成前，不删除 legacy 写路径或开启 write flag。
