...
| P1 | 巨型文件拆分 | permissions/data.ts/postgres-schema 等 10+ 个 >1500 行文件（全仓最大 feishu.ts 10,597 行已拆，e4ba456） | 中 | 🟡 |
