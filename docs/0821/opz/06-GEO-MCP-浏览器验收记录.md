...
- GEOFlow 应用层已升级到提交 `74f4864`；app/queue/scheduler/reverb 使用同一 manifest `sha256:666d366cb039d8b97f33003d6dadc7a37b07f667914c73df3bb27b89525ce7de`，web 保持 `sha256:1aab4a139e8f57c97f5eeaf6b057a96d2d9b2059012ed6f3e900ef1c3ec5a19a`，五个容器均为 `healthy`。
...
2. 清理名称严格匹配 live E2E 约定的历史测试夹具，再发布 `GEOFlow Docker Live mt6a7ruh` 私有目录项；Endpoint 为精确批准的 `http://127.0.0.1:18080/mcp`。
3. 配置加密保存的 Authorization 凭据，选择在线 Codex Runtime，并等待连接状态变为 `ready`。
4. 声明并发现六个带命名空间的工具：`create`、`status`、`autosave`、`validate`、`publish`、`delete`；发布和删除均标为高风险并要求确认。
5. 通过 `create=agent` 深链接打开创建页，以空白模板创建 `GEO Manager mt6a7ruh`，备注名为 `GEO 管理员工 mt6a7ruh`，绑定同一 Runtime，并显式选中本次新员工核对详情。
...
| live E2E 测试夹具 | 本轮清理 1 名员工 / 1 个目录项，最终保留 1 / 1；临时登录会话由 `afterEach` 在成功和失败路径统一回收 |
| 浏览器 console error / warning | 0 / 0 |
| `requestfailed` | 0 |
| HTTP 4xx / 5xx | 0 / 0 |
| 移动端横向溢出 | 0 px |
| 页面 DCL / load | 1313 ms / 1343 ms |
| 桌面 / 移动 LCP | 1216 ms / 1668 ms |
...
| Playwright | 1 passed，38.4 s（总耗时 39.3 s） |
...
3. 使用显式 `confirmation=DELETE` 调用 `delete`，按租户安全回收 13 个历史回归项目及其未被任务引用的知识库。
4. 调用 `geoflow.enterprise_knowledge.create` 创建租户隔离的企业知识项目。
5. 每 2 秒调用 `status`，直到异步队列明确进入 `reviewing`，最长等待 120 秒。
6. 调用 `autosave` 保存人工审核稿，调用 `validate` 校验。
7. 使用显式 `confirmation=PUBLISH` 调用 `publish`。
8. 按本次唯一 `taskId` 核对 AgentSpace 工具审计，再将任务置为 completed。

最终结果：

| 业务实体 / 证据 | 值 |
| --- | --- |
| AgentSpace task | `task-geo-live-mt6aaher`，状态 `completed` |
| 历史夹具回收 | 项目 `1-13` 全部经 MCP 删除成功；回归数据由 13 项目 / 11 知识库收敛为 1 / 1 |
| GEOFlow enterprise project | `14`，发布前状态 `reviewing`，最终 `published` |
| Knowledge base | `13` |
| Knowledge chunks | `3` |
| Embedding | models `embedding-vision` 实际路由返回，provider `dofe-models-api-local`，原始维度 `2048`；三个分块均写入 `vector(3072)` |
| 成功工具审计 | 六类工具全部存在 `succeeded` 记录；13 次删除逐次留痕 |
...
9. GeoFlow `tools/list` 原先向只读令牌暴露写工具名称。现按令牌 scope 过滤目录；修复时真实容器验证写令牌可见 51 个工具，只读令牌仅见 26 个且不包含任务创建和文章发布；本轮新增删除能力后写目录为 52 个。
...
GeoFlow 对应提交：`ca3eeba`（按凭证隔离限流）、`cd9a7ff`（发布竞态保护）、`41cb1c6`（无 pgvector 列兼容）、`0015eb5`（URL 导入并发保护）、`308c62f`（工具发现权限过滤）、`94c220e`（恢复 pgvector 存储列）、`bfcbcd5`（限制 MCP SSO 回退超时）、`a045cac`（提前拒绝无效身份令牌）、`380a319`（3072 维 halfvec HNSW 检索）、`74f4864`（企业知识安全删除工具）。中央基础设施对应提交：`fd60ab7`（PostgreSQL 18 pgvector 镜像、备份和扩展对账）。SSO 对应提交：`736cebc`（HttpOnly 会话验证）。
...
| GeoFlow 企业知识删除 / MCP 契约 | 55 passed，498 assertions；Pint 与镜像产物契约通过 |
...
| 真实 AgentSpace -> GEOFlow MCP | 通过，六类工具、13 个历史项目安全删除、3 chunks、六类成功审计 |
| Chromium 桌面/移动端 | 1 passed，38.4 s；LCP 1216/1668 ms；桌面 INP 40 ms；CLS 0/0；Axe 0 violations；0 console issue；0 failed request；0 HTTP 4xx/5xx；0 overflow；夹具最终 1/1 |
| 外部 SSO 隔离 Chromium | 登录 POST 200；authorize 302；callback 307；工作区 200 |
| GeoFlow MCP 权限目录 | 写令牌 52 个且包含企业知识删除；只读令牌继续隐藏写工具 |
| GeoFlow URL 导入并发 | 5 路并发：3 创建、2 限流；清理后 0 条测试记录 |
| managed-node Node 基线 | `v24.19.0`；daemon healthy；真实六工具任务通过 |
