...
- GEOFlow 已按停机排空、外部迁移、启动和 readiness 顺序升级到提交 `a045cac`；app 镜像 manifest 为 `sha256:386d1acbc72a82effcf3201a8b9d8ab1a86858830832c3d2ca1bed0c96899fac`，web 为 `sha256:1aab4a139e8f57c97f5eeaf6b057a96d2d9b2059012ed6f3e900ef1c3ec5a19a`。
...
2. 发布 `GEOFlow Docker Live mt66wlgn` 私有目录项，Endpoint 为精确批准的 `http://127.0.0.1:18080/mcp`。
3. 配置加密保存的 Authorization 凭据，选择在线 Codex Runtime，并等待连接状态变为 `ready`。
4. 声明并发现五个带命名空间的工具：`create`、`status`、`autosave`、`validate`、`publish`。
5. 打开 AI 员工创建页，以空白模板创建 `GEO Manager mt66wlgn`，备注名为 `GEO 管理员工 mt66wlgn`，绑定同一 Runtime，并显式选中本次新员工核对详情。
...
| `requestfailed` | 0 |
| 移动端横向溢出 | 0 px |
| 页面 DCL / load | 821 ms / 821 ms |
| Playwright | 1 passed，41.0 s（总耗时 41.9 s） |
...
| 业务实体 / 证据 | 值 |
| --- | --- |
| AgentSpace task | `task-geo-live-mt6705z6` |
| GEOFlow enterprise project | `8`，发布前状态 `reviewing` |
| Knowledge base | `7` |
| Knowledge chunks | `3` |
...
13. 浏览器证据原只断言新员工存在，桌面截图却仍展示旧员工详情。用例现显式点击本次创建员工并验证详情面板后截图，桌面和移动端证据均直接显示 `GEO 管理员工 mt66wlgn`。

GeoFlow 对应提交：`ca3eeba`（按凭证隔离限流）、`cd9a7ff`（发布竞态保护）、`41cb1c6`（无 pgvector 列兼容）、`0015eb5`（URL 导入并发保护）、`308c62f`（工具发现权限过滤）、`94c220e`（恢复 pgvector 存储列）、`bfcbcd5`（限制 MCP SSO 回退超时）、`a045cac`（提前拒绝无效身份令牌）。中央基础设施对应提交：`fd60ab7`（PostgreSQL 18 pgvector 镜像、备份和扩展对账）。SSO 对应提交：`736cebc`（HttpOnly 会话验证）。
...
| Chromium 桌面/移动端 | 1 passed，41.0 s；0 console issue；0 failed request；0 overflow |
...
| PostgreSQL / pgvector | PostgreSQL healthy；vector 0.8.6；3/3 新分块向量非空；2048 原始维度补齐至 3072 存储维度 |
...
- 中央 PostgreSQL 已提供 pgvector 0.8.6 和 `vector(3072)` 列，GeoFlow 的精确余弦距离检索已真实执行并返回分数。pgvector 0.8.x 的 HNSW/IVFFlat 常用 `vector` 维度上限为 2000，当前 3072 维存储列不建 ANN 索引；大规模 ANN 仍需选择不超过 2000 维的存储模型或评估半精度/降维方案。
