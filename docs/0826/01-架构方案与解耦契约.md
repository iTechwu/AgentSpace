...
>
> 简化原则（2026-08-26 更新）：Docker 化部署下，Connector 默认使用开放出站网络，不要求第三方 MCP 修改代码或接入专用代理；严格 egress 仅作为按连接启用的可选增强。
...
- MCP Connector 独立部署、独立进程/容器，默认使用可访问外部网络的 Docker bridge；需要高合规场景时再切换 restricted network/egress proxy。
- AgentRouter 只处理 harness 生命周期、模型协议和通用 `ToolSurface`，不出现 MCP URL、MCP secret、MCP catalog 类型。
- Runtime 默认将无 secret 的标准 HTTP MCP endpoint 作为 opaque ToolSurface 配置交给 provider；需要 secret、stdio 或统一会话时才通过版本化 Connector API，provider 永不接触凭据。
...
| MCP egress network | 默认开放出站；可选按连接限制 host/port、TLS 和 proxy | Control Plane DB、其他 runtime | MCP team |
| Provider sandbox | 工作目录、通用 ToolSurface client、已授权 MCP endpoint | MCP secret、控制面 token、未授权外网 | AgentRouter team |

Connector 的网络策略以 connection release 为单位生成。默认只执行协议、超时、响应大小、凭据和租约校验；启用 restricted 模式时再执行 DNS 解析、私网地址拒绝、TLS/CA、重定向、速率和并发限制。provider 看到的只是短期 `toolSession` 句柄。

### 推荐的最简部署模型

1. 第三方 MCP 项目按原生文档构建镜像，AgentSpace 不修改其代码。
2. Connector 与 MCP 容器加入同一个可出站 Docker bridge network；MCP 自己管理 SDK、依赖、外部 API key 和上游重试。
3. AgentSpace 只保存 catalog、镜像/endpoint、能力、owner 和审计引用；任务执行通过标准 MCP endpoint 或 Connector `/mcp` 会话调用。
4. 只有 workspace 明确要求时，才为该连接设置 `networkMode=restricted` 并绑定 egress proxy。网络治理从默认路径移出，避免把所有 MCP 接入都变成基础设施改造。
...
3. **网络边界**：Docker 化部署默认使用可出站 bridge，MCP 自主处理 API key、重试和目标筛选；仅在 workspace 明确启用 `restricted` profile 时，才增加 host/port/path、TLS CA、速率或专用 proxy 约束。`MCP_EGRESS_ENFORCE=true` 仍不得回退到 runtime network。
...
**决策**：MCP 服务默认在自己的 Docker 网络中直接出站，并由 MCP 自己控制目标与凭据；Connector 作为需要统一会话、secret 隔离或审计的可选数据面，restricted egress 仅按需启用。
