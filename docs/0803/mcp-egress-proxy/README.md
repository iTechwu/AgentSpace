...
- remote daemon 已将 CLI 安装收敛到 Runtime 私有持久化 home，并把相同目录挂载到托管 Runtime Docker；托管容器内的 MCP task gateway 使用隔离 bridge 的宿主机网关地址，不再发布不可达的宿主机回环地址。
- MCP 市场已提供工作区私有服务发布入口，支持不可变版本、HTTPS endpoint/allowlist、工具、配置字段、密钥字段与数据域声明。
...
| [04-远程Runtime-CLI与MCP生命周期.md](./04-远程Runtime-CLI与MCP生命周期.md) | remote 安装位置、Docker 挂载、任务调用链和运维检查 |
...
3. Runtime 的 MCP 调用必须是 `Provider -> task gateway -> daemon -> mcp-egress-proxy -> remote MCP`。宿主机 Provider 使用 loopback；托管 Docker Provider 使用隔离 bridge gateway。Provider 不获得 endpoint、proxy lease 或 OAuth token。
