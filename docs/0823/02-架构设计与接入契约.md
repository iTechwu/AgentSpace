...
上游 wire 扩展已先落地 session cancel/resume/approval-policy/close/per-session endpoint overlay 与版本协商；下一阶段只在真实 carrier 验证后补 turn cancel ACK、逐请求 approval，再实现 AgentSpace 的 bounded connection manager。
...
2. P2b：上游 fork 已提供 session cancel、resume、approval-policy、close、per-session environment 与版本协商；仍需 turn cancel ACK、逐请求 approval，再实现 AgentSpace 的 bounded connection manager、provider session 映射和崩溃恢复。真实载体和恢复证据到位前，相关 capability 必须保持 false。
