...
> 决策状态（2026-08-23）：独立 provider、`deepseek_native`、headless P0 已 Accepted 并实现；JSON-RPC P2a 为 Proposed，P2b 受上游 wire 阻断；ACP 暂不实施。
...
当前 wire 只有 `initialize`、`session/prompt`、`shutdown`，通知只有 `session.event`、`session.status`、`subagent.started`、`subagent.finished`。它没有版本协商、turn cancel、session close、approval request/response；服务端对新 session id 调用 `agents.create`，当前证据只支持同一进程内复用，不能证明进程重启后 resume。npm `@deepseek-ai/dsh` 也不包含生产 `dsh-jsonrpc-agent` 载体；载体由 Python runtime wheel/单文件构建链提供。

因此 P2 分两段：

1. P2a：先交付固定版本的独立 runtime 载体和受控 Cordis config；`initialize.serverInfo`、stdout purity、通知排序和关闭梯子通过后，才开启同进程多轮与事件映射。超时/取消通过关闭整个 runtime 进程实现，并使该 worker 上的会话全部失效。
...
2. profile：发布门禁要求 `dsh --profile headless --help` 成功且无端口监听；当前实现尚未取得真实镜像证据。
3. credential：通过内存中的 HTTP probe 或一次受限 model request 验证 key，key 不进入 argv。
4. model：发布门禁要求明确验证 `deepseek-v4-flash` 与 `deepseek-v4-pro`；当前 health 只对 `${DEEPSEEK_BASE_URL}/models` 做认证请求，不能替代模型级 canary。
