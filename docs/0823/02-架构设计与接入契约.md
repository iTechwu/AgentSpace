...
env:
  ...
  DEEPSEEK_BASE_URL: managed Models gateway `/v1` endpoint; standalone direct override is test-only
...
上游 wire 扩展已落地 session cancel/resume/approval-policy/close/turn-cancel-ack/approval-request-response/per-session endpoint overlay 与版本协商；下一阶段在真实 carrier 验证后实现 AgentSpace 的 bounded connection manager。
...
2. P2b：上游 fork 已提供 session cancel、resume、approval-policy、close、turn cancel ACK、approval request/response、per-session environment 与版本协商；下一步实现 AgentSpace 的 bounded connection manager、provider session 映射和崩溃恢复。真实载体和恢复证据到位前，相关 capability 必须保持 false。
...
| `DEEPSEEK_BASE_URL` | Models `MODELS_GATEWAY_BASE_URL` 派生的 `/v1` 地址 | managed runtime 必须使用 Models gateway；仅 standalone 协议测试允许直连官方 endpoint |
...
托管 runtime 必须沿用 `buildProviderEnv` 的“清空宿主 provider key、再注入 task-scoped credential”策略；`DEEPSEEK_API_KEY` 必须是 Models `RuntimeCredential` 一次性签发的 secret，`DEEPSEEK_BASE_URL` 必须由 Models `MODELS_GATEWAY_BASE_URL` 派生，二者都加入 redaction 和 managed-key 清单。发布 gate 拒绝任何非 gateway endpoint 覆盖。
