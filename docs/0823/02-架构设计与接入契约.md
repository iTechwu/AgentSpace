...
2026-08-24 更新：上游 fork 已提供 `session/cancel`、`session/resume`、`session/approval-policy` 及仅允许非机密 `DEEPSEEK_BASE_URL` 的 per-session environment overlay。AgentSpace one-shot 目前只在外部 abort 前 best-effort 发送 cancel，随后关闭整个 runtime；版本协商、turn cancel ACK、逐请求 approval、session close、bounded worker、provider session 映射和真实恢复证据仍未交付。
...
当前 wire 只有 `initialize`、`session/prompt`、`shutdown`，通知只有 `session.event`、`session.status`、`subagent.started`、`subagent.finished`。它没有版本协商、turn cancel、session close、approval request/response，也不能为新 session 更新进程环境；服务端对新 session id 调用 `agents.create`，当前证据只支持同一静态进程环境内复用，不能证明进程重启后 resume。AgentSpace 每个远端任务的 Skill、credential attribution、gateway 日志路径均为 task-scoped 环境，直接复用进程会把前一任务环境带入后一任务，因此在上游提供 per-session environment 或等价隔离接口前不得开启跨任务 bounded worker。npm `@deepseek-ai/dsh` 不包含生产 `dsh-jsonrpc-agent`；固定上游 tag `dsh-v0.1.1-rc.2` 已提供 `deepseek-harness-runtime-bin` wheel 与 `dsh-jsonrpc-agent-pkg-<platform>-<arch>` 单文件构建链，发布 bundle 同时要求相邻的 `-rg`，macOS 还要求 `-spawn-helper`。本工作站尚未构建或安装该 bundle。
...
2. P2b：先在上游 fork 扩展并版本化 cancel、approval、session close、resume 协议，再实现 AgentSpace 的 bounded connection manager、provider session 映射和崩溃恢复。没有这些 wire 方法时，相关 capability 必须保持 false。
