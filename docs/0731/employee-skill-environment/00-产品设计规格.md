...
## 0. 当前实现审查标注（2026-07-31，复核至工作区 `2b54ef4` 及本轮工作区实现）

**审查基线**：初稿基线为 `6980985`（12:07），文档初稿记录于 13:36；其后工作区继续实现：`be01834`（14:49）完整落地员工 Skill 环境变量配置 MVP、`439d8f4`（16:05）敏感配置与审计增强、`2b54ef4`（22:22）额外变量与「请求管理员配置」转交入口。本节为对当前工作区 `2b54ef4` 及本轮未提交改动的复核：六项原「部分实现」能力均已闭环，额外变量与转交入口亦已实现；本轮进一步落地深度标注清单中的 ①③④⑤⑦ 五项（picker 就绪状态、客户端预检、升级差异明细、设置页概览、端到端安全证明文档）。状态含义：`已实现` 指已有业务代码和可达路径，`部分实现` 指后端或界面只完成一段链路，`未实现` 指当前无产品入口或业务能力。本版对 §1–§9 的全部规格条目做了逐条深度标注（`▶` 标记实现偏差）。
...
**已验证测试**：服务与 Runtime 相关用例单并发 45/45 通过（`agent-skill-requirements.test.ts` 37 + `requirements.test.ts` 8），覆盖保留 Key 声明阻断、受管凭据 Key 阻断、保存前冲突、同值复用、复用来源解析、单 Key 删除、密钥轮换、需求升级差异与 `expired` 状态、`invalidDeclarations`、`credentialKeyWarnings`、capability 真实能力目录比对、`resolveSkillProjectWorkDirSync` 共识、`statusDetail` 状态码、摘要不泄露密钥、运行时合并与兜底冲突，以及**额外变量**（持久化/注入/敏感加密/保留前缀拒绝/删除）与**转交入口**（`skill-configuration-request.test.ts` 3/3：管理员通知、请求者排除、未知 Skill 拒绝，审计只含键名不含值）；`packages/daemon/src/task-context-skill-env.test.ts` 与 `agent-router/utils.test.ts` 10/10 通过（含按注入 Key 值脱敏）；`features/agents/actions.test.ts` 17/17、`features/dashboard/data.test.ts` 25/25 通过，`agents-page-client.test.tsx` 中「已安装 Skill 维护入口不泄露密钥」「capability-only UI 交互」「额外变量增/删」用例通过。相关数据库测试必须顺序执行；并行执行会因共享测试数据库产生版本冲突和重复键，不能作为并行隔离的回归证明。另：工作区存在与本能力无关的 pre-existing UI 用例失败（`agents-page-client.test.tsx` 知识/Feishu/引导类 14 项、`skills-page-client.test.tsx` 预设市场 1 项、workspace settings 路由 7 项），失败均为测试夹具文本不匹配或运行时模式断言，与本功能链路无关。
...
| 观察 | 对用户的影响 | 设计含义 |
| --- | --- | --- |
| 在「添加 Skill」中只能看名称、描述、来源、文件数 | 用户选择后才知道需要配置什么 | 在选择前展示就绪状态和要求摘要。▶ **已解决**：`SkillPickerModal` 新增 `statusBySkillId`，每行展示 `就绪，可安装 / 需配置 N 项 / 运行环境不兼容`（按声明要求计数与绑定 Runtime Provider 计算） |
| 安装弹窗把环境变量称作「普通配置」，没有变量来源、敏感级别和注入范围 | 用户不知道填写的值是不是环境变量、会不会生效 | 使用明确的「环境变量」概念和两类字段。▶ **已解决**：弹窗使用「环境变量 / 密钥环境变量」分组、敏感（加密保存）开关，并说明注入范围（仅此员工 x Skill、密钥不回显） |
...
1. **先声明、再配置、后安装。** 未声明的变量不在安装向导中出现；未通过预检的 Skill 不进入“已就绪”。▶ **已实现**：弹窗仅渲染声明项；`getSkillRequirementBlockers` 门禁 readiness。
2. **配置归属清楚。** 默认作用域是「此员工的此 Skill」，不会因同名变量自动泄露给同一员工的其他 Skill。▶ **已实现**：`employee + skill` 键存储；复用需显式选择且不展示值。
3. **秘密默认不回显。** API Key、Token、Password、Secret 等变量默认视为密钥；保存后只显示“已配置”、更新时间和操作者。▶ **已实现**：声明解析时 `SECRET_KEY_PATTERN` 自动升级 config→secret；额外变量同名自动加密；保存后仅展示 `updatedAt/updatedBy`。
4. **安装成功等于运行可用。** UI 的“已安装”只在变量已持久化、运行时绑定可解析、预检通过后出现。▶ **已实现**：保存前 readiness 校验 + 任务启动 input-bundle 409 双重断言；Runtime 离线置 `awaiting_validation` 不显示为就绪。
5. **渐进披露。** 没有要求的 Skill 一步安装；有要求的 Skill 才进入配置流程。Demo 阶段不引入工作区级全局变量、模板批量分发和复杂继承。▶ **已实现**：`SkillPickerModal.onSelect` 无要求直接 `onSetSkillIds`；有要求才开配置弹窗。
...
| 角色 | 核心任务 | 可见/可操作范围 | 现状 |
| --- | --- | --- | --- |
| 工作区管理员 | 安装、配置、轮换密钥、查看失败原因 | 全部变量的元数据；可写普通变量和密钥 | **已实现**（`assertWorkspaceRoleForContext(…,"admin")` 门禁安装/维护/删除/轮换） |
| 员工管理者 | 为其有管理权的员工装 Skill、维护非密钥配置 | 变量名称、状态、来源；可写普通变量 | **部分实现**：`assertCanManageEmployeeForActorSync` 支持管理其名下员工，但安装含要求 Skill 的 action 仍限 `admin`；未按「管理者」角色拆分 |
| 密钥管理员（后续角色） | 提供或轮换密钥，不修改 Skill 绑定 | 密钥字段和审计记录，不可读取旧值 | **未实现**（Demo 阶段统一由管理员承担） |
| 普通成员 | 使用已就绪员工 | 只看“可用/需管理员处理”，不见变量名和值 | **已实现**：`redactSkillRequirementsForViewer` 仅见状态/计数；未就绪 Skill 提供「请求管理员配置」入口 |
...
不新增顶级“环境变量”导航。变量的第一责任人是员工与 Skill 的绑定关系，留在这个上下文中能避免“这个 Key 到底被谁使用”的问题。员工详情的设置页可增加一个只读的“环境变量概览”，用于汇总有问题的 Skill，点击后回到对应的 Skill 卡片。▶ **现状：已实现**——员工详情 `settings` 标签页新增只读「环境变量概览」，列出非 `ready` 状态的 Skill（状态 chip + 查看按钮切回 Skills 标签），非管理员同样只读到脱敏摘要。
...
- 用户可在“高级”中为已安装 Skill 添加额外变量。额外变量必须显式关联到一个 Skill，且受同样的命名、长度和密钥策略校验；不提供员工全局自由变量。**现状：已实现。** `SkillRequirementsModal` 的「额外变量」区块支持添加/移除键名（大写命名、`DOFE_AGENT_*` 保留前缀与重复声明校验、匹配 `TOKEN/SECRET/API_KEY` 模式的键自动加密）；`extraKeys` 随配置持久化并注入任务环境，可标记敏感后加密保存，亦可通过单 Key 删除移除。
...
1. **添加 Skill 列表**：每一行显示 `就绪，可安装`、`需配置 2 项` 或 `运行环境不兼容`；点击有要求的 Skill 打开配置界面而不是立即绑定。▶ **已实现**：`SkillPickerModal` 每行按 `statusBySkillId` 展示状态 chip（`skillRequirementPreview` 计算声明 config/secret 计数，按 `record.boundProvider` 判定 Provider 兼容）；无要求 Skill 仍一步安装。
2. **配置并安装**：采用三段式单页向导，而不是长表单弹窗。
   ...
   ▶ **部分实现（实现形态不同）**：采用**单页弹窗分区**（`SkillRequirementsModal`：模型运行时 / 项目工作目录 / 环境变量 / 密钥环境变量 / 额外变量），而非三段式向导；安装检查与确认安装合并为一个表单，「复用已有值」、敏感开关、凭据 Key 提示均已就位，但没有独立“确认安装”摘要步。
3. **预检失败**：不关闭界面；失败项就地标红并给出可执行动作，如“更换执行引擎”“填入 `NOTION_API_TOKEN`”“请求管理员配置”。已填密钥仅保存在浏览器内存，离开前明确提醒用户。▶ **已实现（客户端预检 + 服务端兜底）**：弹窗 `noValidate` + `validateBeforeSubmit` 在提交前检查缺失的声明 config/secret 与额外变量，就地 `aria-invalid` 标红并显示错误文案、顶部汇总 banner，不调用 `onConfirm`（弹窗不关闭）；服务端 `normalizeSkillRequirementConfiguration`/`getSkillRequirementBlockers` 仍兜底。服务端级错误（如跨 Skill 冲突）仍经 toast 呈现后关闭弹窗。「请求管理员配置」已作为 Skill 卡片入口实现（§5.3），非弹窗内联动作。
...
| 状态 | 视觉与文案 | 主操作 | 现状 |
| --- | --- | --- | --- |
| 已就绪 | 绿色状态点，“2/2 环境变量已配置” | 管理环境变量 | **已实现**（`status="ready"`） |
| 需配置 | 警告状态点，“缺少 `NOTION_API_TOKEN`” | 完成配置 | **已实现**（`needs_configuration` + blockers 明细） |
| Runtime 不兼容 | 错误状态点，“需要 Codex，当前为 Claude” | 查看安装检查 | **已实现**（`runtime_incompatible` + capability/provider blocker） |
| 等待验证 | 中性状态点，“Runtime 离线，配置已保存” | 重试验证 | **已实现**（`awaiting_validation`）；「重试验证」复用执行引擎 Provider 验证动作，非独立每 Skill 按钮 |
| 已过期 | 警告状态点，“Skill 更新后新增 1 项要求” | 查看变更并更新 | **已实现**：卡片 chip 显示「已过期 · 新增 N 项（/移除 M 项）」；维护弹窗经 `upgradeAddedKeys`/`upgradeRemovedKeys` 展示新增/移除键**差异明细 banner** |

“管理环境变量”是安装界面的维护模式：普通变量可编辑，密钥只支持替换/删除；显示作用域、最近更新人和时间、关联的 Skill。删除被声明为必填的变量会二次确认，并将 Skill 立即标为“需配置”。▶ **已实现**：维护弹窗展示 `updatedAt/updatedBy` 与 Skill 名；单 Key 删除有二次确认；删除必填项后 `deleteAgentSkillRequirementKeySync` 重算状态为 `needs_configuration`。「作用域（员工 x Skill）」由员工详情上下文隐含，未在弹窗内显式标注。
...
- **权限不足**：显示键名、类型和“由管理员配置”；不显示值输入，也不以模糊的“安装失败”结束。**现状：已实现。** 含要求的安装 action 限管理员；非管理员经 `redactSkillRequirementsForViewer` 仅见状态与计数（不含环境变量明细）。未就绪的 Skill 卡片对非管理员提供「请求管理员配置」入口，经 `requestSkillRequirementConfigurationSync` 通知工作区管理员/所有者（排除请求者）并写入不含值的审计事件（键名与数量）。
...
2. 秘密只能在服务端/受控 Runner 解密，写入该次子进程的内存环境或短生命周期权限受限文件；不得写入 `SKILL.md`、`skill.config.json`、任务产物、终端日志、审计 note 或浏览器响应。**现状：已实现。** 静态 Skill 文件不含值、数据库加密且 UI 不回显；解密值仅经 authenticated daemon 的 input bundle 传输，执行 stdout/stderr 经 `buildEnvValueRedactions` 按注入 Key 值脱敏后落库。**端到端安全证明已成文**：见 [01-端到端安全证明.md](01-端到端安全证明.md)（威胁模型、生命周期、分阶段控制、脱敏覆盖矩阵、残余风险）。
...
- 员工详情的 Skill 列表展示要求摘要与五种配置状态。▶ **已实现**：已绑定 Skill 卡片 chip 展示五种状态（§5.2）。
- 有要求的安装使用“配置并安装”流程；已安装 Skill 可再次打开维护模式。▶ **已实现**：`SkillRequirementsModal` 的 `install`/`manage` 双模式。
- 支持现有 `config:` 和 `secret:` 声明，按员工 x Skill 保存；所有密钥加密、不可回显、可替换。▶ **已实现**：AES-256-GCM 加密、永不回显、轮换路径。
...
- Runtime 离线、Provider 不兼容、缺变量、加密键缺失、依赖失败均有可理解的状态与修复入口。▶ **已实现**：`awaiting_validation` / `runtime_incompatible` / `needs_configuration` / 加密键缺失定向文案 / `dependencyInstallStatus` chip。
...
| 场景 | 必须结果 | 现状 |
| --- | --- | --- |
| 无 requires 的 Skill | 一次点击安装，不出现配置负担 | **已实现**（`onSelect` 无要求直接绑定） |
| `config` + `secret` Skill | 管理员在一次流程中完成配置；值在任务进程可用 | **已实现**（弹窗收集 → 加密存储 → 任务 env 注入） |
| 安装后替换 Token | 旧值不可读取；下一次任务只使用新值；有审计记录 | **已实现**（轮换写加密存储 + `workspace.agent_skill_secret_rotated` 独立事件） |
| 缺少变量/Runtime 不兼容 | Skill 不显示“已就绪”，界面指出具体键或兼容条件 | **已实现**（`needs_configuration` 列缺键、`runtime_incompatible` 列兼容条件） |
| Runtime 离线 | 配置可保存，但状态明确为等待验证 | **已实现**（`awaiting_validation`） |
| Skill 需求升级 | 旧绑定可见但标为已过期；新增项可定位并补配 | **已实现**（`expired` + 维护弹窗可补配；差异明细列表见 §5.2 已过期行备注） |
| 安全验证 | 变量值不出现在页面数据、产物、日志和审计记录 | **已实现**（UI 不回显、`buildEnvValueRedactions` 按注入 Key 值脱敏、审计只含键名） |

## 8. 可用性与度量

首轮可用性测试招募 5-8 名实际负责配置数字员工的管理员，覆盖首次安装和密钥轮换。任务为：为一位员工安装声明 `NOTION_DATABASE_ID` 与 `NOTION_API_TOKEN` 的 Skill；识别 Runtime 不兼容并修复；在安装后替换 Token。▶ **现状：未实现（度量目标，规划中）**——Demo 阶段未执行可用性测试，也未采集结构性指标。
...
1. 抽取并返回员工 x Skill 的配置摘要和状态，先让员工详情可观察。▶ **已实现**（`readAgentSkillRequirementSummarySync` → `WorkspaceAgentRecord.skillRequirements`）。
2. 增加维护型配置接口与界面；保留目前的安装接口作为兼容入口。▶ **已实现**（维护弹窗 + `install`/`manage` 双模式 + 单 Key 删除/轮换）。
3. 把员工级配置接入任务启动器，加入一次真实注入预检和脱敏日志。▶ **已实现**（`resolveAgentSkillEnvironment` + `collectSkillReadinessBlockers` + input-bundle 409 + `buildEnvValueRedactions`）。
4. 为状态计算、权限、密钥不回显、运行注入、Skill 要求升级和冲突分别增加测试。▶ **已实现**（services 45 + 转交 3 + daemon 10 + web 相关用例）。
5. 在测试环境使用 CI/Jenkins 路径验证：配置、运行、轮换、失败重试和日志脱敏的完整闭环。▶ **未执行**：本工作站禁用本地 Jenkins 部署（见项目 CLAUDE.md「Local Jenkins Prohibition」），测试环境闭环验证待独立 CI 环境执行。

当前代码已闭环：保留 Key 阻断、保存前冲突、readiness 回传、已安装维护入口、绑定 Runtime 的受管凭据反向反查、配置期复用引导、单 Key 删除与独立轮换审计、模型/工作目录的执行接入、任务级 readiness 重校验、按注入 Key 值脱敏、为已安装 Skill 添加「额外变量」（§4）与非管理员「转交管理员配置」入口（§5.3）均已实现（对应 §0 六项能力复核）。

**深度标注后的八项跟进，其中五项已实现**：① 添加 Skill 选择器展示每项就绪状态（§5.1.1，`SkillPickerModal.statusBySkillId`）；③ 预检失败就地标红、不关闭弹窗（§5.1.3，客户端 `validateBeforeSubmit` + `noValidate`）；④ 已过期 Skill 差异明细列表（§5.2，`upgradeAddedKeys/upgradeRemovedKeys` banner）；⑤ 员工详情设置页「环境变量概览」（§3，只读汇总问题 Skill）；⑦ 端到端安全证明文档（§6.2，[01-端到端安全证明.md](01-端到端安全证明.md)）。

**仍留待后续（含决策说明）**：② 三段式安装向导（§5.1.2，当前单页弹窗已分区完成同等功能，改造为向导风险大于收益，建议保留单页形态）；⑥ 员工管理者/密钥管理员角色拆分（§3，MVP 文档明示「仅管理员可安装含要求的 Skill」，拆分属权限面扩展，建议 Beta 阶段随角色矩阵实施）；⑧ 可用性度量与 CI/Jenkins 测试环境闭环验证（§8/§9.5，本工作站禁用 Jenkins 且需真实用户与独立 CI 环境）。
