...
这条链路已经覆盖“谁调用、哪个任务创建了哪个 Job、每个阶段能否恢复展示”和“产物如何安全进入原 AI 员工的存储边界”。它尚未覆盖 Job Worker 自动执行完整视频流水线、自动触发产物桥接、最终视频聊天预览以及 models 权威账单归因。
...
- OpenMontage `tests/openmontage`：55 个通过；Python `compileall` 通过。
...
2. **Artifact Bridge Worker 接线与 E2E**：实际 executor 自动下载 Job 输入、发布 final MP4，并验证 100MB 以上文件不进入 MCP JSON；失败重试不得重复登记或遗留临时文件。
...
当前准确表述是：**AgentSpace 已具备对已部署 OpenMontage 的受管 MCP 控制面、可信 Job 绑定、双向 Artifact Bridge 和持久化阶段/产物事实能力；自动视频生产与统一计费仍未完成。**
