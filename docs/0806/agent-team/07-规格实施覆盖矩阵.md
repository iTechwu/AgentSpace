...
| ID | 规格要求 | 状态 | 实现证据 | 验证 / 门禁 |
| --- | --- | --- | --- | --- |
| B01 | workspace 隔离表、唯一约束、状态转换与 Definition 变更审计 | `DONE` | schema `111`、`packages/db/src/workflows/`；创建/更新与审计同事务，更新只记录持久化前后实际不同的字段；111 修复早期 110 误 reparent 的运行触发器链接 | 纯函数/静态/仓储测试；实库受 T03 约束 |
...
1. 在隔离 PostgreSQL 上执行 T03，先验证 schema `111`、事务幂等、并发领取和真实 legacy dry-run。
