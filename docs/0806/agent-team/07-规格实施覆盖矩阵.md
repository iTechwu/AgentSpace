...
1. 在隔离 PostgreSQL 上执行 T03，先验证 schema `116`、114/115 旧实例省略 `history_sequence` 的兼容写入、事务幂等、并发领取、审批扫描公平重试和真实 legacy dry-run。
