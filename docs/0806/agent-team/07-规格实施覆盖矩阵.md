...
更新时间：2026-08-06  
代码基线：`67482af`（后续文档提交不改变本表所列代码证据）  
...
| P07 | 有限重试、指数退避、暂停、恢复、取消、失败传播 | `DONE` | `retries.ts`、`coordinator.ts`、运行控制 UI | Control/Coordinator 单测 |
...
| U05 | 运行流程、节点状态、时间线、Artifact 与断点补偿 | `DONE` | `workflow-run-client.tsx`、events route | Web 测试；实库刷新链路受 T04 约束 |
...
| B03 | 事件名称白名单、去重键与受保护入口 | `DONE` | `events.ts`、`/api/internal/workflows/events` | Event/Route 单测 |
| B04 | lease、Outbox、幂等消费、恢复扫描 | `DONE` | DB outbox、worker、recovery | Worker/Service 单测；真实故障受 T05 约束 |
| B05 | 复用 `agent_task_queue` 和 daemon completion | `DONE` | `dispatcher.ts`、`completion.ts`、daemon routes | 定向单测；全链路受 T03/T04 约束 |
| B06 | 原子并发领取，避免超限 dispatch | `DONE` | DB `FOR UPDATE` claim + governance snapshot | 纯函数/类型检查；并发实库受 T03 约束 |
| B07 | 最终失败、审批拒绝、Join 与下游终态传播 | `DONE` | `coordinator.ts` | Coordinator 决策单测 |
| B08 | secret 不入图、诊断脱敏、低基数指标 | `DONE` | `security.ts`、`observability.ts` | 安全/观测单测 |
...
| ID | 门禁 | 状态 | 已有证据 | 关闭条件 |
| --- | --- | --- | --- | --- |
| T01 | Domain/Service/Web/Worker 定向测试 | `DONE` | 2026-08-06：Domain 9、Service 46、Web/API/双入口 38、Worker 2，全部通过 | 数据库与浏览器链路分别由 T03/T04 管理 |
| T02 | Workflow 相关类型检查、ESLint、diff check | `DONE` | Services/Web/Worker types、Web test types、ESLint、`git diff --check` 通过 | 最终文档提交后复跑 diff/link check |
| T03 | PostgreSQL 仓储、事务、并发和迁移集成 | `ENV-BLOCKED` | 测试文件和 schema 已存在 | 提供 `DOFE_AGENT_TEST_DATABASE_URL` / 隔离 PostgreSQL 后全绿 |
