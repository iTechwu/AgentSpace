...
| ID | 规格要求 | 状态 | 实现证据 | 验证 / 门禁 |
| --- | --- | --- | --- | --- |
| B01 | workspace 隔离表、唯一约束、状态转换与 Definition 变更审计 | `DONE` | schema `114`、`packages/db/src/workflows/`；创建/更新与审计同事务，更新只记录持久化前后实际不同的字段；112 修复 Trigger reparent，113 增加审批截止时间列与索引，114 增加 `approval_scan_after` 公平重试游标，避免损坏审批永久占满扫描批次；指定审批人和风险等级仍存于审批记录的 application JSON metadata | 纯函数/静态/仓储测试；实库受 T03 约束 |
...
1. 在隔离 PostgreSQL 上执行 T03，先验证 schema `114`、事务幂等、并发领取、审批扫描公平重试和真实 legacy dry-run。
