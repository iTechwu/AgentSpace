...
| ID | 规格要求 | 状态 | 实现证据 | 验证 / 门禁 |
| --- | --- | --- | --- | --- |
| B01 | workspace 隔离表、唯一约束、状态转换与 Definition 变更审计 | `DONE` | schema `112`、`packages/db/src/workflows/`；创建/更新与审计同事务，更新只记录持久化前后实际不同的字段；112 是 **Trigger reparent 数据修复**（置空 110 误 reparent 的 `workflow_run.trigger_id`），审批限时/指定审批人/风险等级不是 schema 列而是审批记录的 application JSON metadata | 纯函数/静态/仓储测试；实库受 T03 约束 |
...
1. 在隔离 PostgreSQL 上执行 T03，先验证 schema `112`、事务幂等、并发领取和真实 legacy dry-run。
