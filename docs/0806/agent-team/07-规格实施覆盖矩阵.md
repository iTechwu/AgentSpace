...
代码证据：本轮复审基线 `a7a7b8bb`，最终范围以本目录所在已提交版本为准
...
| P02 | 手动、一次性、每日、Cron、受控事件触发 | `DONE` | `scheduler.ts`、`events.ts`、内部 events route、Builder “立即运行”入口；服务端只接受权威 active/manual Trigger | Scheduler/Event/Materialization/Builder/Action 单测 |
...
| ID | 规格要求 | 状态 | 实现证据 | 验证 / 门禁 |
| --- | --- | --- | --- | --- |
| U01 | 目标、触发、流程、治理、预览五步向导；owner、频道通知目标、真实发布版本与草稿修订展示 | `DONE` | `workflow-builder-client.tsx`、`workflow-data.ts` | Builder Vitest + 类型检查 |
...
| U05 | 运行流程、节点状态、时间线、Artifact、权限操作与断点补偿 | `DONE` | `workflow-run-client.tsx`、服务端 `canControl` 投影、events route；审批等待可暂停，轮询按 sequence/请求代次防回退 | Web 测试；实库刷新链路受 T04 约束 |
...
| ID | 规格要求 | 状态 | 实现证据 | 验证 / 门禁 |
| --- | --- | --- | --- | --- |
| B01 | workspace 隔离表、唯一约束、状态转换与 Definition 变更审计 | `DONE` | schema `116`、`packages/db/src/workflows/`；创建/更新与审计同事务，更新只记录持久化前后实际不同的字段；112 修复 Trigger reparent，113 增加审批截止时间列与索引，114 增加 `approval_scan_after` 公平重试游标，115 增加运行历史序号，116 增加旧实例写入兼容触发器；完整 keyset v2 索引改为事务后并发创建，迁移入口统一取得 115/116 advisory lock，并会清理中断构建留下的 invalid/not-ready 同名索引；指定审批人和风险等级仍存于审批记录的 application JSON metadata | 纯函数/静态/仓储测试；实库受 T03 约束 |
...
| B12 | 提交后崩溃、跨 workspace 对账与 HTTP 即时重驱 | `DONE` | 全局 stale journal scan、committed crash-gap recovery、幂等 finalizer、共享 `projectTaskCompletion`；Web/本地 Daemon 在频道消息已存在时仍按稳定消息/分片键补偿 Feishu outbox，入队失败保留 pending journal/staging；独立 `/api/cron/task-commit-reconcile` 由 local/remote maintenance 调用 | Reconciliation/Route/CLI Outbox 定向测试；outbox DB 幂等测试受 T03/T05 约束 |
...
| ID | 门禁 | 状态 | 已有证据 | 关闭条件 |
| --- | --- | --- | --- | --- |
| T01 | Domain/Service/Web/Worker 定向测试 | `DONE` | 2026-08-07：Domain 37 项、Workflow Service 27 项、Feishu Outbound 26 项、Web Completion/Reconcile/Cron 18 项、Web Builder/Run/Action 29 项、CLI Completion/Client 6 项及部署/发布配置 9 项通过 | 数据库、TOS 与浏览器链路分别由 T03/T04 管理 |
| T02 | Workflow 相关类型检查、ESLint、diff check | `DONE` | 2026-08-07：根 `pnpm typecheck` 全绿（deps/web/cli/daemon/workflow-worker）；本轮修复 presentation 重复翻译键（TS1117）、triggerPayload `none` 泄漏（TS2322）、channels 合并遗留类型错误、daemon `resolveManagedServiceConnection` 返回类型（TS2339）后通过。ESLint/diff check 仍需最终提交前复跑 | 最终提交前复跑 diff/link/lint check |
| T03 | PostgreSQL 仓储、事务、并发、提交恢复和迁移集成 | `ENV-BLOCKED` | Journal/Queue/Message/Route/Reconciliation/Recovery 测试文件已存在；本地仓储测试覆盖旧写入触发器和固定锁重试 | 提供 `DOFE_AGENT_TEST_DATABASE_URL` / 隔离 PostgreSQL 后全绿；并发启动 115/116 迁移竞争者，确认 metadata 不回退、无死锁，并验证并发索引有效 |
| T04 | Playwright 五场景与最新 UI 视觉/无障碍复核 | `ENV-BLOCKED` | E2E 场景已编写、类型可检查 | 隔离测试库 + 本地/测试站点执行通过 |
| T05 | 真实 1000 Trigger / 100 Run / 并发 20 与故障演练 | `ENV-BLOCKED` | 有受限模拟脚本，不等同真实容量证据 | P95 ≤60s、零重复、清理成功、恢复证据齐全；单独记录同 workspace 跨 Workflow 创建 Run 的序号锁等待 |
| T06 | 目标用户可用性验证 | `EXTERNAL` | 任务脚本和指标已定义 | 5-8 人指标达标，P0/P1 清零 |
| T07 | 产品、工程、安全、运维签字 | `EXTERNAL` | 清单已预留 | 所有责任人和证据链接填写完成 |

## 8. 当前剩余路径

1. 在隔离 PostgreSQL 上执行 T03，先验证 schema `116`、114/115 旧实例省略 `history_sequence` 的兼容写入、115/116 双迁移锁、事务后并发 keyset v2 索引、事务幂等、并发领取、审批扫描公平重试和真实 legacy dry-run；混合路由分别验证 114/115/旧 116 与 v3 游标的刷新/续页行为。
