...
代码证据：本轮深审起点 `d0e8b5e`，最终范围以本目录所在提交为准
...
| B11 | 输出副作用检查点、Workspace 提升和业务投影分阶段提交 | `DONE` | complete route、local daemon、`task_commit_journal`、共享 staging | Completion/Output Bundle 定向测试；原子性受 T03 约束 |
| B12 | 提交后崩溃、跨 workspace 对账与 HTTP 即时重驱 | `DONE` | 全局 stale journal scan、committed crash-gap recovery、幂等 finalizer | Reconciliation 测试入口已补；实库受 T03/T05 约束 |
...
| ID | 门禁 | 状态 | 已有证据 | 关闭条件 |
| --- | --- | --- | --- | --- |
| T01 | Domain/Service/Web/Worker 定向测试 | `DONE` | 2026-08-07：无数据库依赖的 Completion、Output Bundle、Workflow Data、Reconcile Route、Daemon Client 等定向测试通过 | 数据库与浏览器链路分别由 T03/T04 管理 |
