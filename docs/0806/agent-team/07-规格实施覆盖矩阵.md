...
代码证据：本轮复审基线 `379884d1`，最终范围以本目录所在已提交版本为准
...
| ID | 规格要求 | 状态 | 实现证据 | 验证 / 门禁 |
| --- | --- | --- | --- | --- |
| U01 | 目标、触发、流程、治理、预览五步向导；owner 展示与频道通知目标 | `DONE` | `workflow-builder-client.tsx`、`workflow-data.ts` | Builder Vitest + 类型检查 |
...
| ID | 规格要求 | 状态 | 实现证据 | 验证 / 门禁 |
| --- | --- | --- | --- | --- |
| B01 | workspace 隔离表、唯一约束、状态转换与 Definition 变更审计 | `DONE` | schema `109`、`packages/db/src/workflows/`；创建/更新与审计同事务 | 纯函数/静态/仓储测试；实库受 T03 约束 |
...
| ID | 门禁 | 状态 | 已有证据 | 关闭条件 |
| --- | --- | --- | --- | --- |
| T01 | Domain/Service/Web/Worker 定向测试 | `DONE` | 2026-08-07：Domain 37 项、Workflow Service 24 项、Feishu Outbound 26 项、Web Completion/Reconcile/Cron 18 项、Web Builder/Run/Action 28 项、CLI Completion/Client 6 项及部署/发布配置 9 项通过 | 数据库、TOS 与浏览器链路分别由 T03/T04 管理 |
