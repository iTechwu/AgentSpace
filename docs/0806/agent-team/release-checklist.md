...
| Schema 116 滚动兼容 | 代码与仓储测试通过，环境待演练 | 所有 schema 入口按顺序取得 advisory lock 115/116；迁移后用 114/115 形状执行不含 `history_sequence` 的 Run INSERT，确认触发器分配非空序号；模拟中断并发建索引产生 invalid/not-ready 同名索引，确认下一次启动先清理后重建；确认 `idx_workflow_run_workspace_created_v2` 为 valid/ready 后再排空旧 Web/Worker。禁止并行运行绕过锁的手工 DDL。**锁拓扑（契约）**：仅 schema 迁移入口（db-init、运行时自动迁移、SQLite/PG 迁移）持有迁移锁 `[115,116]`；后台自愈（history 回填、`SET NOT NULL`、在线建索引，入口 `ensurePostgresConcurrentIndexes`）持有**独立**的后台锁 `117`，二者刻意解耦——后台维护**不得**包入 `[115,116]`，否则贯穿大表回填/建索引（可达分钟级）会令第二实例冷启动 `getDatabase→ensureRuntimeSchema` 取不到 `[115,116]`，超过 `acquireRuntimeSchemaLock` 的 ~9s 超时后抛 "schema lock busy" 致冷启动/首请求失败。前向版本守卫在 `117` 锁内首句复检即可（117 已串行化所有后台维护实例）。**滚动升级**：已部署旧版本的后台维护仅取 117、锁外检查，与新版本迁移并行时可能对已升级库跑旧维护 DDL——由 `runBackgroundMaintenance` 的幂等+前向安全兜底（`WHERE history_sequence IS NULL` / `GREATEST…WHERE <` / `isHistorySequenceNullable` 门控 / `CREATE INDEX CONCURRENTLY IF NOT EXISTS`；schema 迁移累加不删 `history_sequence` 列），锁串行化无法修补已部署旧二进制；建议升级窗口先让单实例完成回填（置 `schema_116_history_backfill_complete=true`）再滚动其余，避免首批多实例在 flag 未置时并发冷启动 |
