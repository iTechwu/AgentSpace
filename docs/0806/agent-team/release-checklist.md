...
| PostgreSQL schema 版本 | 代码为 `112`，环境待核对 | `packages/db/src/postgres-schema.ts`；测试库 `app_metadata.schema_version` 必须等于 `112`（112 在 111 基础上落地审批限时 `expiresAt`、指定审批人 `reviewerUserId`、风险等级 `risk` 等审批元数据，并修复 110 误 reparent 运行链接） |
