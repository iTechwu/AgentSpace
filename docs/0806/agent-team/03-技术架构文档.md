...
- 频道消息与 Feishu outbox 分开幂等：即使频道消息已存在（`source_task_queue_id` 命中）也必须继续补偿 outbox；外发 outbox 使用 workspace/integration/消息/分片稳定键生成确定性主键，恢复重放只返回已有记录，不重复外发。入队异常不得被吞掉，journal 保持 `projection pending` 以便下一次补偿。
