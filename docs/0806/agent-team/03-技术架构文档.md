...
容量超过目标时优先按 workspace 分片扫描和批量 claim；不得通过无界并发压垮 Runtime。Run 最大并发已由原子 claim 强制执行；预计成本预算在发布前校验。运行历史序号在同一 workspace 内串行分配，以换取严格的提交顺序快照；完整 keyset 排序索引降低续页成本，T05 必须单独验证跨 Workflow 并发创建 Run 的锁等待和 P95。实际 Provider 成本归集和运行中预算熔断为二期。
