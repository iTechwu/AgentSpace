# AI 员工团队编排：业务架构文档

## 1. 业务能力地图

```mermaid
graph TD
  A[团队目标] --> B[Workflow 设计]
  B --> C[触发与调度]
  C --> D[员工任务执行]
  D --> E[产物与证据]
  E --> F[汇聚与交付]
  F --> G[审批与治理]
  G --> H[复盘与模板化]
  B --> I[权限/预算/版本]
  C --> J[可靠性/去重/补偿]
  D --> K[Runtime/Skill/Provider]
```

| 能力域 | L1 能力 | 首期业务结果 |
| --- | --- | --- |
| 计划管理 | 定义、版本、模板、发布 | 可复用且可审计的协作计划 |
| 触发管理 | 手动、日历、事件 | 在正确时间只创建一次 Run |
| 图调度 | 依赖解析、并行、Join | 不依赖人工交接推进步骤 |
| 执行治理 | 员工、Runtime、Skill、预算 | 只在就绪和授权条件满足时执行 |
| 交付管理 | 摘要、Artifact、频道回写 | 根任务有可消费的最终结果 |
| 人类控制 | 审批、暂停、取消、接管 | 高影响动作保持人类决策权 |
| 运营分析 | 运行、成本、失败、SLA | 可定位瓶颈并持续优化模板 |

## 2. 角色与责任矩阵

| 活动 | Workspace 管理员 | 任务负责人 | 员工 owner | 审批人 | AI 员工 |
| --- | --- | --- | --- | --- | --- |
| 创建草稿 | A/R | R | C | I | I |
| 配置员工步骤 | A | R | C | I | I |
| 发布版本 | A/R | R（若被授权） | C | I | I |
| 触发运行 | A | R | I | I | I |
| 执行步骤 | I | A（结果负责） | C | I | R |
| 高影响审批 | I | C | C | A/R | I |
| 节点重试 | A | R | C | I | I |
| 修改员工能力 | A | I | A/R | I | I |
| 删除 Workflow | A/R | C | I | I | I |
| 复盘和模板发布 | A | R | C | C | I |

说明：A=Accountable，R=Responsible，C=Consulted，I=Informed。权限校验最终由服务端执行，不以 UI 隐藏作为安全边界。

## 3. 核心业务对象

```mermaid
erDiagram
  WORKSPACE ||--o{ WORKFLOW : owns
  WORKFLOW ||--o{ WORKFLOW_VERSION : has
  WORKFLOW_VERSION ||--o{ WORKFLOW_NODE : contains
  WORKFLOW_VERSION ||--o{ WORKFLOW_EDGE : connects
  WORKFLOW ||--o{ WORKFLOW_TRIGGER : starts
  WORKFLOW_VERSION ||--o{ WORKFLOW_RUN : materializes
  WORKFLOW_RUN ||--o{ NODE_RUN : executes
  NODE_RUN ||--o{ ARTIFACT : produces
  NODE_RUN }o--|| EMPLOYEE : assigned_to
  NODE_RUN }o--|| AGENT_TASK_QUEUE : delegates
  NODE_RUN ||--o{ APPROVAL : waits_for
```

### 业务不变量

1. Workflow 至少有一个 AI Task Node，且必须存在唯一开始和结束路径。
2. 发布版本必须是 DAG；所有节点都可从开始到达，所有终点都能到达结束。
3. Node 的员工绑定使用稳定 `employeeId`，展示名称只是快照。
4. Run 永远绑定已发布版本；编辑草稿不能改变历史 Run。
5. Node Run 的输入只能来自触发参数、静态配置和已完成上游的显式输出。
6. Join 只消费声明的上游，不读取任意同 workspace 任务或聊天历史。
7. 一个 Node Run 最多有一个活动 attempt；重试必须带递增 attempt 序号和幂等键。

## 4. 生命周期

### Workflow

`draft → validating → published → paused → archived`

- `draft` 可编辑；`validating` 执行依赖、权限和图完整性预检。
- `published` 可被触发；修改必须生成新版本。
- `paused` 不创建新 Run，已运行实例按 Run 策略继续或暂停。
- `archived` 不可重新触发，但历史数据可读。

### Workflow Run

`created → queued → running → waiting_approval → succeeded | partially_succeeded | failed | cancelled | paused`

### Node Run

`pending → ready → queued → running → waiting_approval | retry_wait | succeeded | failed | skipped | cancelled`

状态迁移必须带 `actorType`、`reasonCode`、`occurredAt` 和关联事件 ID。UI 只显示本地化文案，数据库保存稳定代码。

## 5. 端到端业务流程

```mermaid
sequenceDiagram
  participant Owner as 任务负责人
  participant Web as 编排中心
  participant Engine as 编排引擎
  participant Queue as AgentSpace 任务队列
  participant Agent as AI 员工/Runtime
  participant Human as 审批人
  Owner->>Web: 创建并发布 Workflow
  Web->>Engine: 校验图、权限、预算、Runtime
  Engine-->>Web: 发布 Version
  Engine->>Engine: 到期/事件去重，创建 Run
  Engine->>Queue: 投递首批 Node Run
  Queue->>Agent: 现有 AgentRouter 执行
  Agent-->>Engine: 完成事件 + 摘要 + Artifact 引用
  Engine->>Engine: 激活并行节点或 Join
  Engine->>Human: 高影响动作进入审批
  Human-->>Engine: 批准/拒绝
  Engine->>Queue: 投递汇总员工
  Agent-->>Engine: 最终交付
  Engine-->>Owner: 根任务、频道、产物和审计
```

## 6. 业务规则

### 调度规则

- 时区存储在 Trigger 上，时间点统一转 UTC 计算；展示使用用户选择的 workspace 时区。
- 重复触发使用 `workflowId + triggerId + scheduledAt` 去重；同一时点不创建两个 Run。
- 迟到触发默认跳过过期窗口；可配置一次性“补跑最近一次”，不默认追赶全部历史。
- Workflow 暂停时不创建新 Run；恢复后只按未来时间计算。

### 协作规则

- 并行节点只有在所有前置条件满足后才一起进入 ready；同一组内节点互不阻塞。
- Join 默认等待全部上游成功；“允许部分成功”必须在发布前显式确认。
- 汇总员工必须绑定一个稳定员工；不能用“自动选择”替代权限和责任。
- 上游失败时，Join 不得伪装成功；Run 进入失败或部分成功并显示缺失节点。

### 成本和风险规则

- 发布前显示预估最大并发节点数和每次运行预算上限。
- 重试计入预算；超过预算进入暂停并通知负责人。
- 发送外部消息、修改外部数据、写入生产资源等动作须通过现有审批/工具授权。

## 7. 运营模型

运营人员关注四类队列：待发布的草稿、即将触发的计划、阻塞/审批中的运行、需要人工接管的失败节点。每日看板展示成功率、P95 调度延迟、节点失败原因、平均重试次数、成本和人工介入率。

每个 Workflow 都应有 owner、备用负责人、通知频道和停用策略。员工 Runtime 离线、Skill 过期或权限撤销时，下一次触发前预检失败并通知，不创建半执行 Run。

## 8. 现有能力映射

| 现有能力 | 业务架构中的位置 | 处理方式 |
| --- | --- | --- |
| `/calendar` + `ScheduledTask` | Schedule Trigger | 迁移为单节点 Workflow；保留兼容读取 |
| `/automations` + `AutomationRule` | Event Trigger / Action Adapter | 可转换的规则进入版本草稿；复杂 legacy 规则保留适配器 |
| `/task-board` + `TaskRecord` | Root Task 投影 | 根任务显示 Run 总体和下一待办节点 |
| `agent_task_queue` | AI Task Node 的执行委托 | 不改变 Provider claim contract，增加 workflow metadata |
| AgentRouter session | Node Run attempt 的 Runtime 执行上下文 | 一个 Node Run 可拥有多个 attempt，但同一时刻只有一个活动 attempt |
| approvals | Human Approval Node / 工具审批 | 复用现有来源、审计和通知机制 |
| task execution event | 节点和运行事实事件 | 增加 workflowId/versionId/runId/nodeId 关联字段 |

