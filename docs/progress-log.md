...
- **补充（ecd7b8ef，补齐「无校验」缺口）**：保持内联不动，新增静态扫描测试 `apps/web/features/i18n/inline-bilingual-consistency.test.ts`——词法级解析括号/引号/模板插值后校验两类贴错文案（A：en 槽纯中文；B：zh 槽无 CJK 且与 en 不符），含规则探针自测与扫描量下限（>3000）防空转绿灯；模板占位符/大小写等价英文标签/语言代码三类合法模式显式豁免。首扫 4,266 调用点：9 处命中均合法，**无真实贴错文案**。
