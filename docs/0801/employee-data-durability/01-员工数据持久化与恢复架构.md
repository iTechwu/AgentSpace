...
> **最新实施状态（2026-08-03）**
>
> 完整 workspace revision、对象存储 blob、workDir diff/tombstone、任务前 fail-closed 还原、持久 mount、恢复状态机、generation fencing、提交 reconciliation、真实 Skill 依赖安装/任务注入、受管 Skill service 和历史 revision 恢复入口均已落地。剩余项集中在 employeeId 全链迁移、真实销毁/物理恢复验收、恢复 smoke、规模化传输和生命周期策略。详见 [第五轮实施审查](./03-实施审查与后续优化.md)。
>
...
> **实施状态**：【部分实现】`packages/services/src/employees/recovery.ts` 已实现 `allocate → mount_workspace → install_skills → resolve_secrets → health_check → activate` 分 phase 推进，并在每个 mutating phase 前通过 `assertBindingGenerationCurrentSync` 检查 `fromGeneration`；`activateRecoveryBindingSync` 原子切换 `runtime_id` + `generation`。但 `allocate` 阶段仅记录/复用 `runtimeId`，真正的容器创建、workspace 挂载、secret 注入仍由调用方（runtime-provisioning 服务）完成。
...
> **实施状态**：【剩余】当前仅依赖对象存储版本化/数据库集中备份，尚未实现可留存、可审计的外部备份-恢复演练流程与策略配置。
