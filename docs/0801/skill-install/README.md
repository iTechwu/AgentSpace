...
2. 依赖已真实安装+校验；**service 控制面 + managed-node worker + retire 生命周期 + catalog 准入加固已实现**（daemon claim → docker pull/create/start/health → complete 回报 endpointRef，retire 拆除；retire 生产者 + 未引用扫描；SBOM/安全字段/JSON 校验 + docker 加固），服务组件按 binding 裁决；剩余是 secret 注入、egress 强制与真实 uninstall 路径。
