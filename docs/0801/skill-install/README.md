...
2. 依赖已真实安装+校验；**service 控制面 + managed-node worker + retire 生命周期 + catalog 准入加固 + secret 注入 + egress 强制已实现**（daemon claim → docker pull/create/start/health → complete 回报 endpointRef，retire 拆除；retire 生产者 + 未引用扫描；SBOM/安全字段/JSON 校验 + docker 加固；secrets 认证通道注入 + 脱敏；egress 内部网络/DNS+hosts），服务组件按 binding 裁决；剩余是真实 uninstall 路径。
...
6. skill-install 与 service operation 的租约/心跳/fencing/崩溃重排、service managed-node worker（docker 生命周期 + retire）、retire 生命周期 + catalog 准入加固（schema v77）+ rollback_class 感知退役（schema v78）+ secret 注入（schema v79）+ egress allow-list 强制均已实现并有专属测试（catalog 15/15 + bindings 17/17 + secrets 4/4 + db 9/9 + runtime 19/19 + worker 8/8 + 路由 4/4 + maintenance 5/5）；**剩余**：真实 uninstall 路径。
