...
6. skill-install 与 service operation 的租约/心跳/fencing/崩溃重排、service managed-node worker（docker 生命周期 + retire）、retire 生命周期 + catalog 准入加固（schema v77）+ rollback_class 感知退役（schema v78）+ secret 注入（schema v79）+ egress allow-list 强制 + uninstall 路径均已实现并有专属测试（catalog 15/15 + bindings 17/17 + secrets 4/4 + db 9/9 + runtime 19/19 + worker 8/8 + uninstall 3/3 + 路由 4/4 + maintenance 5/5）。
