...
2. 依赖已真实安装+校验；**service 控制面 + managed-node worker + retire 生命周期已实现**（daemon claim → docker pull/create/start/health → complete 回报 endpointRef，retire 拆除；`queueManagedSkillServiceRetireSync` 生产者 + 未引用扫描入维护循环），服务组件按 binding 裁决；剩余是 catalog 准入加固、secret/network policy 与真实 uninstall 路径。
...
6. skill-install 与 service operation 的租约/心跳/fencing/崩溃重排、service managed-node worker（docker 生命周期 + retire）、retire 生产者 + 生命周期扫描均已实现并有专属测试（DB 7/7 + services 14/14 + runtime 12/12 + worker 6/6 + 路由 3/3 + maintenance 5/5）；**剩余**：catalog 准入加固、secret/network policy、真实 uninstall 路径。
