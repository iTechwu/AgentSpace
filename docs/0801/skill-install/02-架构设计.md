...
entrypoint 可通过 `configKeys` 声明最多 64 个大写配置键。任务快照只携带键名；daemon 从任务已解析的 Skill config/secret 中逐键筛选，并先从 Provider 环境中剥离这些键，再生成随机短时 JSON 文件，只读挂载到 `/run/secrets/dofe-skill-config.json` 并以 `DOFE_SKILL_CONFIG_FILE` 告知脚本。值不得进入 Docker argv、artifact、Provider 进程环境、Provider workspace 或未声明键集合；缺键、非法键或挂载失败均在执行前阻断。当前网络仍固定为 `none`，在 managed-node egress broker 能强制 host allowlist 前，不接受联网声明。
