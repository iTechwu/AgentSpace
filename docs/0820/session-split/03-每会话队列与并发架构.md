# 每会话队列与并发架构
> **实现状态（本次会话）**：§2/§3 串行键与领取规则已实现（Lane 串行 + 无 Lane 回退 legacy）；§4 Router/Provider 隔离已实现；§5 Runtime capacity（`runtime_task_capacity` 表 + 容量门控 + `capacity_wait` 投影）已实现；§6 公平性按 Lane head 保证 FIFO（跨 Lane 按 priority）；§7 幂等竞态（跨 Lane 并行、同 Lane 串行、跨 runtime 互斥）单测覆盖。§7.3 双连接并发 claim race 注入、§9 可观测指标仍未实现（属运维增强，非上线阻断）。
