# 每会话队列与并发架构
> **实现状态（本次会话）**：§2/§3 串行键与领取规则已实现——chat task 有 `execution_lane_id` 时按 Lane 串行，无 Lane 时回退 legacy（`requester+employee` / `router_session_id`）；单测覆盖「跨 Lane 并行、同 Lane 串行」。§5 Runtime capacity、§6 公平性 aging、§7 幂等竞态（双连接 claim race）、§9 可观测指标未实现。
