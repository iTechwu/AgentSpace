...
test("workspace storage helpers resolve workspace-scoped persistent paths", () => {
  assert.equal(getSystemWorkspaceDataDirPath(), join(resolvedTempRoot, "data", "workspaces", "__system__"));

  const historyDir = getWorkspaceChannelHistoryDirPath("workspace-mars");
  assert.equal(historyDir, join(resolvedTempRoot, "data", "workspaces", "workspace-mars", "channel-history"));
  assert.equal(existsSync(historyDir), true);

  assert.equal(
    getWorkspaceDaemonRemoteStagingDirPath("queue 7", "workspace-mars"),
    join(resolvedTempRoot, "data", "workspaces", "workspace-mars", "daemon-remote-staging", "queue-7"),
  );
});

test("daemon execution helpers scope workdirs by workspace and execution kind", () => {
  const stateDir = getLocalDaemonStateDirPath();
  assert.equal(stateDir, join(resolvedTempRoot, "data", "daemon"));
  ...
  assert.equal(
    ...
    join(resolvedTempRoot, "data", "daemon", "workspaces", "workspace-mars", "workdirs", "queue-7"),
  );
  ...
  assert.equal(
    ...
    join(resolvedTempRoot, "data", "daemon", "workspaces", "workspace-mars", "remote-workdirs", "queue-7"),
  );
});
