...
function buildFeishuIntegrationSetupGuide(input: {
  ...
  return {
    ...
    commands: {
      ...
      strictLiveSmoke: `pnpm run smoke:feishu -- --env-file scripts/feishu/.env --live --strict-live --evidence ${FEISHU_SMOKE_EVIDENCE_PATH} --json --require-todo120-native`,
      verifyOpenApiEvidence: `pnpm run smoke:feishu -- --verify-evidence ${FEISHU_SMOKE_EVIDENCE_PATH} --json`,
      verifyBotAddedPayload: `pnpm run smoke:feishu -- --verify-bot-added-payload ${FEISHU_BOT_ADDED_PAYLOAD_PATH} --bot-added-payload-evidence ${FEISHU_BOT_ADDED_PAYLOAD_EVIDENCE_PATH} --json`,
