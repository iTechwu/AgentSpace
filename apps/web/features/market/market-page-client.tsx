                ...
                <div className="market-runtime-box">
                  ...
                  <div className={`market-installability market-installability--${selectedInstallability.status}`} role="status">
                    <span className={`status-chip status-chip--${selectedInstallability.status === "installable" ? "positive" : selectedInstallability.status === "needs_configuration" ? "warning" : "danger"}`}>
                      {runtimeAppInstallabilityStatusLabel(selectedInstallability.status, tx)}
                    </span>
              ...
              <li className="mcp-service-group" key={key}>
                ...
                <ul className="mcp-runtime-connection-list">
                  {installations.map((installation) => {
                    ...
                    const operationActive = data.operations.some((operation) => operation.runtimeId === installation.runtimeId && operation.appSource === installation.source && operation.appName === installation.name && isActiveCapabilityOperationStatus(operation.status));
...
function runtimeAppReadinessLabel(
  ...
  return runtimeAppInstallabilityStatusLabel(installability.status, tx);
}
...
function runtimeAppInstallability(
  ...
  return projectRuntimeAppInstallability(app.installability, runtime.cliReadiness);
}
